...
async function processWordFile(content) {
  ...
  try {
    ...
    return { output, selfCitationData, totalNumberOfSelfCitationsFound };
  } catch (error) {
    throw error;
  }
}
...
async function NumberselfWordFile(content) {
  var abstractSection = keywordSection = keyword_2_Section = keyword_3_Section = keyword_4_Section = introductionSection = conclusionSection = referenceSection = false;
  ...
  try {
    ...
    for (let i = 0; i < paragraphs.length; i++) {
      ...
      if (abstractSection){
        if(check_paragraph && !keywordSection){keywordSection = paragraph.toLowerCase().startsWith("keyword");}
        if(check_paragraph && !keyword_2_Section){keyword_2_Section = paragraph.toLowerCase().startsWith("key word");}
        if(check_paragraph && !keyword_3_Section){keyword_3_Section = paragraph.toLowerCase().startsWith("index");}
        if(check_paragraph && !keyword_4_Section){keyword_4_Section = paragraph.toLowerCase().startsWith("indices");}
        if(check_paragraph && !introductionSection){introductionSection = paragraph.toLowerCase().includes("introduction");}
        if(!check_paragraph && !conclusionSection){conclusionSection = paragraph.toLowerCase().includes("conclusion"); }
        if(check_paragraph && conclusionSection && !referenceSection){referenceSection = paragraph.toLowerCase().includes("reference");}

        check_paragraph = true;

        if(keyword_2_Section || keyword_3_Section || keyword_4_Section || introductionSection){
        ...
        if(check_paragraph){
          ...
          const highlightedParagraph = paragraph.replace(
            combinedPattern, '<span style="background-color: yellow;">$&</span>'
          );
          output += highlightedParagraph + "\n\n";
        }
      } else {
...
const author = {};
const highlightAndCountMatches = (text, patterns, selfcitationdata, refCitationData) => {
...
app.post("/api/upload", upload.single("file"), (req, res) => {
  ...
  extracted
    .then(async function (doc) {
      ...
      const referenceCitationInAbstractandConclusion = await NumberselfWordFile(extractedText);
